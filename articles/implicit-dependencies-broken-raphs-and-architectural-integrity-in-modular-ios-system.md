...
This article explains why implicit dependencies occur, why they are so hard to eliminate, how Tuist detects them, and how to automate detection with a robust prebuild script. We will also cover an SPM oriented alternative for teams using Swift Package Manager.
...
## Why implicit dependencies cannot be fully prevented

Eliminating implicit dependencies would require changing fundamental assumptions in Xcode and the Swift toolchain. Several forces prevent this from happening:
...
* The ecosystem expects linking to be permissive rather than strict

This topic is covered directly in the implicit dependencies overview from Pedro mentioned earlier, which explains why Apple cannot easily change these behaviors without breaking compatibility.
...
You cannot change these realities. The build system is permissive by design. The only sustainable way to handle implicit dependencies is to detect them and enforce explicit responsibility before they accumulate.
...
This is not an approximation. It is a verification step that ensures the declared dependency graph matches the actual one.
...
This plugin only works for SPM based projects, which makes it a natural fit for pure Swift package setups and a complement to Tuist for hybrid ecosystems.
...
Implicit dependencies are a systemic artifact of the iOS build system. Xcode and Swift were not designed to enforce strict dependency declarations, which means implicit imports will always be possible. The right response is not to trust developers to remember them, but to enforce them automatically.

Tuist and SPM tooling provide the means. By integrating checks into builds and CI, teams ensure that modularization is backed by enforcement rather than convention.
